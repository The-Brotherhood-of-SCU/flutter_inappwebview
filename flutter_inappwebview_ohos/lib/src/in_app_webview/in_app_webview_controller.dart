...
import '../ohos_serialization.dart';
...
WebResourceError _webResourceErrorFromChannelMap(Map<String, dynamic> map) {
  final description = map['description']?.toString() ?? '';
  final typeRaw = map['type'];
  WebResourceErrorType type;
  if (typeRaw is String) {
    type = WebResourceErrorType.fromValue(typeRaw) ??
        WebResourceErrorType.UNKNOWN;
  } else if (typeRaw is int) {
    type = WebResourceErrorType.fromNativeValue(typeRaw) ??
        WebResourceErrorType.UNKNOWN;
  } else if (typeRaw is num) {
    type = WebResourceErrorType.fromNativeValue(typeRaw.toInt()) ??
        WebResourceErrorType.UNKNOWN;
  } else {
...
WebResourceResponse _webResourceResponseFromChannelMap(
    Map<String, dynamic> map) {
  return WebResourceResponse.fromMap(map) ??
      WebResourceResponse(
        ...
        contentEncoding: map['contentEncoding']?.toString(),
        headers: map['headers']?.cast<String, String>(),
      );
}

MediaPlaybackState? _parseMediaPlaybackState(Map<dynamic, dynamic>? map) {
  if (map == null) return null;
  return MediaPlaybackState.fromNativeValue(map['state'] is int
      ? map['state'] as int
      : (map['state'] is String
          ? int.tryParse(map['state'] as String)
          : null));
}
    ...
    switch (call.method) {
      ...
      case "onReceivedError":
        if ((webviewParams != null &&
          ...
          WebResourceError error = _webResourceErrorFromChannelMap(
      ...
      case "onReceivedHttpError":
        if ((webviewParams != null &&
          ...
          WebResourceRequest request = WebResourceRequest.fromMap(
              call.arguments["request"].cast<String, dynamic>())!;
          WebResourceResponse errorResponse = _webResourceResponseFromChannelMap(
      ...
      case "onScrollChanged":
        if ((webviewParams != null && webviewParams!.onScrollChanged != null) ||
            _inAppBrowserEventHandler != null) {
          int x = (call.arguments["x"] as num).toInt();
          int y = (call.arguments["y"] as num).toInt();
          if (webviewParams != null && webviewParams!.onScrollChanged != null)
      ...
      case "onZoomScaleChanged":
        if ((webviewParams != null &&
            ...
            _inAppBrowserEventHandler != null) {
          double oldScale = (call.arguments["oldScale"] as num).toDouble();
          double newScale = (call.arguments["newScale"] as num).toDouble();
      ...
      case "onPermissionRequestCanceled":
        if ((webviewParams != null &&
          ...
          PermissionRequest permissionRequest =
              permissionRequestFromOhos(arguments);
        ...
        if ((webviewParams != null &&
          ...
          if (webviewParams != null) {
            if (webviewParams!.onPermissionRequest != null)
              return permissionResponseToOhos(
            ...
            return permissionResponseToOhos(await _inAppBrowserEventHandler!
      ...
      case "onCallJsHandler":
        ...
        if (_javaScriptHandlersMap.containsKey(handlerName)) {
          ...
          try {
            final callback = _javaScriptHandlersMap[handlerName]!;
            return jsonEncode(await (callback is JavaScriptHandlerCallback
                ? callback(args)
                : callback()));
...
"""))?.cast<Map<dynamic, dynamic>>() ?? [];
  ...
  Future<void> loadUrl(
    ...
    // Match Android: POST loadUrl delegates to postUrl. Nested Uint8List body
    // inside urlRequest.toMap() is unreliable on the OHOS method channel.
    final method = urlRequest.method?.toUpperCase();
    final body = urlRequest.body;
    final url = urlRequest.url;
    if (method == 'POST' && body != null && url != null) {
      await postUrl(url: url, postData: body);
      return;
    }
  ...
  Future<void> injectJavascriptFileFromUrl(
    ...
    // OHOS: script.onload/onerror via JS bridge is unreliable (especially on
    // about:blank). Fetch on the Dart side, inject inline, invoke callbacks
    // directly. Fall back to the bundled test jquery asset when CDN is blocked.
    if (urlFile.isScheme('http') || urlFile.isScheme('https')) {
      await _injectRemoteJavascriptFileFromUrl(
  ...
  Future<void> _injectRemoteJavascriptFileFromUrl(
      {required WebUri urlFile,
      ScriptHtmlTagAttributes? scriptHtmlTagAttributes}) async {
    final id = scriptHtmlTagAttributes?.id;
    String? source;
    try {
      source = await _fetchUrlAsString(urlFile).timeout(const Duration(seconds: 8));
    } catch (_) {
      source = null;
    }

    final urlStr = urlFile.toString();
    if (source == null &&
        urlStr.contains('jquery-3.3.1.min.js') &&
        !urlStr.contains('notawebsite')) {
      try {
        source =
            await rootBundle.loadString('test_assets/js/jquery-3.3.1.min.js');
      } catch (_) {
        source = null;
      }
    }

    final idLiteral = id != null ? jsonEncode(id) : null;
    if (source != null) {
      final inject = StringBuffer('(function(){var s=document.createElement("script");');
      if (idLiteral != null) {
        inject.write('s.id=$idLiteral;');
      }
      inject.write('s.text=${jsonEncode(source)};');
      inject.write('if(document.body!=null){document.body.appendChild(s);}})();');
      await evaluateJavascript(source: inject.toString());
      scriptHtmlTagAttributes?.onLoad?.call();
      return;
    }

    final injectError = StringBuffer('(function(){var s=document.createElement("script");');
    if (idLiteral != null) {
      injectError.write('s.id=$idLiteral;');
    }
    injectError.write('if(document.body!=null){document.body.appendChild(s);}})();');
    await evaluateJavascript(source: injectError.toString());
    scriptHtmlTagAttributes?.onError?.call();
  }

  Future<String> _fetchUrlAsString(WebUri urlFile) async {
    final client = HttpClient();
    try {
      final request = await client.getUrl(Uri.parse(urlFile.toString()));
      final response = await request.close();
      if (response.statusCode >= 400) {
        throw HttpException('HTTP ${response.statusCode}',
            uri: Uri.parse(urlFile.toString()));
      }
  ...
  void addJavaScriptHandler(
    ...
    // The OHOS bridge does not provide frame metadata for the new callback API.
    if (callback is! JavaScriptHandlerCallback &&
        callback is JavaScriptHandlerFunction) {
      throw UnsupportedError(
          'OHOS JavaScript handlers must accept List<dynamic>, '
          'not JavaScriptHandlerFunctionData.');
    }
  ...
  Future<Uint8List?> takeScreenshot(
    ...
    final data = await channel?.invokeMethod('takeScreenshot', args);
    if (data == null) {
      return null;
    }
  ...
  Future<void> setSettings({required InAppWebViewSettings settings}) async {
    ...
    args.putIfAbsent('settings', () => webViewSettingsToOhos(settings));
  ...
  Future<InAppWebViewSettings?> getSettings() async {
    ...
    if (settings != null) {
      ...
      return webViewSettingsFromOhos(settings as Map<String, dynamic>);
    }
  ...
  Future<OhosPrintJobController?> printCurrentPage(
    ...
    Map<String, dynamic> args = <String, dynamic>{};
    args.putIfAbsent("settings", () => printSettingsToOhos(settings));
    String? jobId =
  ...
  Future<double?> getZoomScale() async {
    Map<String, dynamic> args = <String, dynamic>{};
    final result = await channel?.invokeMethod('getZoomScale', args);
...
(function() {
  ...
  Future<void> postWebMessage(
    ...
    Map<String, dynamic> args = <String, dynamic>{};
    args.putIfAbsent('data', () => message.data);
    args.putIfAbsent('type', () => message.type.toNativeValue());
    args.putIfAbsent('targetOrigin', () => targetOrigin.toString());
    // 传 ports 的信息作为简单类型数组（避免嵌套Map序列化问题）
    if (message.ports != null && message.ports!.isNotEmpty) {
      List<dynamic> portInfos = [];
      for (var port in message.ports!) {
        var portMap = port.toMap();
        portInfos.add(portMap["webMessageChannelId"]);
        portInfos.add(portMap["index"]);
      }
      args.putIfAbsent('ports', () => portInfos);
    }
  ...
  Future<MediaPlaybackState?> requestMediaPlaybackState() async {
    ...
    return _parseMediaPlaybackState(stateMap);
  }
  ...
  void dispose({bool isKeepAlive = false}) {
    if (!isKeepAlive && id != null) {
      try {
        _staticChannel.invokeMethod('disposeWebView', {'viewId': id});
      } catch (e) {
        // Native WebView may already be disposed by PlatformView lifecycle.
      }
    }
